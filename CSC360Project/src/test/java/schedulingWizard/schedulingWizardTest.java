...
class schedulingWizardTest
{
	...
	{
		Session registrar = new Session("Jacob Johnson", "Registrar", "Registrars Office");
		Scheduler regScheduler = registrar.validate();
		boolean ans = regScheduler.checkCourseConstraints();
		assertTrue(ans);
		
		Session deptHead = new Session("Dr. Bradshaw", "Dept Head", "Computer Science");
		Scheduler deptScheduler = deptHead.validate();
		assertThrows(IllegalArgumentException.class, () -> {
			deptScheduler.checkCourseConstraints();
