...
class schedulingWizardTest
{
	...
	void setUp() throws Exception
	{
		...
		ArrayList<String> depts = new ArrayList<String>(Arrays.asList("History", "Biology", "Computer Science", "Music", "Math", "Axe Throwing"));
		...
		ArrayList<Course> CSCMasterSchedule = new ArrayList<Course> ();
		history.addToMasterSchedule("Computer Science", CSCMasterSchedule);
		
		//creating an example ARS Master Schedule
		ArrayList<Course> ARSMasterSchedule = new ArrayList<Course> ();
		history.addToMasterSchedule("Studio Art", ARSMasterSchedule);
		
		//creating an example BIO Master Schedule
		ArrayList<Course> BIOMasterSchedule = new ArrayList<Course>();
		history.addToMasterSchedule("Biology", BIOMasterSchedule);
	...
	@Test
	void testAddAndRemoveCourseAndFinalize()
	{
		...
		/*
		//finalize for correct dept
		deptHead.finalize("Computer Science");
		ArrayList<String> finalizedCourses = new ArrayList<String>(Arrays.asList("Computer Science"));
		ArrayList<String> CSC360GenEdTags = new ArrayList<String>(Arrays.asList("D3"));
		
		//check CS has been added to list of finalized courses
		assertEquals(finalizedCourses, History.listOfFinalizedDepts);
		
		//check a new course cannot be added to finalized schedule
		assertThrows(IllegalArgumentException.class, () -> {
			deptHead.addNewCourse("Software Design", "CSC360", CSC360GenEdTags, 
				"Olin201", "Spr25", "Bradshaw", "Computer Science","8:00AM", "9:00AM",10);
			});
		
		//finalize for wrong dept
		assertThrows(IllegalArgumentException.class, () -> {
			deptHead.finalize("Math");
			});
		*/
		//test add false capacity
		deptHead.setNewFalseCapacity(courseRetrieved, 5);
		assertEquals(courseRetrieved.falseCapacity,5);
		
		assertThrows(IllegalArgumentException.class, () -> {
			deptHead.setNewFalseCapacity(courseRetrieved, 100);
			});
	}
	...
	@Test
	void testRoomAndTimeConflicts()
	{
		ArrayList<String> CSC360GenEdTags = new ArrayList<String>(Arrays.asList("D3"));
		Session session = new Session("Jacob Johnson", "Registrar", "Registrars Office");
		Scheduler registrar = session.validate();
		Course BIO100 = registrar.addNewCourse("Course1", "BIO100", CSC360GenEdTags, "Olin 200", "Spr25", "Carr", "Biology", "9:00AM", "10:00AM", 10);
		Course BIO100SameRoom = registrar.addNewCourse("Course2", "BIO100SameRoom", CSC360GenEdTags, "Olin 200", "Spr25", "Carr", "Biology", "12:00PM", "1:00PM", 10);
		Course BIO100SameTime = registrar.addNewCourse("Course3", "BIO100SameTime", CSC360GenEdTags, "Olin 201", "Spr25", "Carr", "Biology", "9:00AM", "10:00AM", 10);
		
		//check there is a conflict (room)
		Map<Course,Course> coursesThatShouldConflict = new HashMap<Course,Course>();
		coursesThatShouldConflict.put(BIO100, BIO100SameRoom);
		ArrayList<Course> deptSchedule = history.getScheduleForDept("Biology");
		Map<Course,Course> coursesThatDoConflict = registrar.checkRoomConflicts(deptSchedule);
		assertThrows(IllegalArgumentException.class, () -> {
			registrar.finalize("Biology");
			});
		
		assertEquals(coursesThatShouldConflict,coursesThatDoConflict);
		//check there is no conflict (room)
		registrar.removeCourse("Biology", "BIO100SameRoom");
		Map<Course,Course> coursesThatDoConflict2 = registrar.checkRoomConflicts(deptSchedule);
		Map<Course,Course> coursesThatShouldConflict2 = new HashMap<Course,Course>();
		assertEquals(coursesThatShouldConflict2,coursesThatDoConflict2);

		//check there is a conflict (time)
		Map<Course,Course> coursesThatShouldConflict3 = new HashMap<Course,Course>();
		coursesThatShouldConflict.put(BIO100, BIO100SameTime);
		Map<Course,Course> coursesThatDoConflict3 = registrar.checkRoomConflicts(deptSchedule);
		assertEquals(coursesThatShouldConflict3,coursesThatDoConflict3);
		
		//check there is no conflict (time)
		registrar.removeCourse("Biology", "BIO100SameTime");
		ArrayList<Course> checkSchedule = history.getScheduleForDept("Biology");
		for(Course course : checkSchedule)
		{
			System.out.println(course.courseCode);
		}
		
		Map<Course,Course> coursesThatDoConflict4 = registrar.checkRoomConflicts(deptSchedule);
		Map<Course,Course> coursesThatShouldConflict4 = new HashMap<Course,Course>();
		assertEquals(coursesThatShouldConflict4,coursesThatDoConflict4);
		registrar.finalize("Biology");
		ArrayList<String> finalizedDepts = new ArrayList<String>(Arrays.asList("Biology"));
		assertEquals(History.listOfFinalizedDepts,finalizedDepts);
	}
